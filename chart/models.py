...
class PopularWorksChart(Chart):
    ...
    def get_query_set(self):
        if self.range:
            ...
            group_by = 'record'
        return (
            self._filter_by_range(Work.objects.exclude(title=''))
            .annotate(factor=models.Count(group_by, distinct=True))
            .order_by('-factor', 'title')
        )
...
class ActiveUsersChart(Chart):
    ...
    def get_query_set(self):
        return (
            self._filter_by_range(User.objects)
            .annotate(factor=models.Count('history'))
            .order_by('-factor', 'username')
        )
