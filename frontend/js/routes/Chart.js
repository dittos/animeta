...
function renderDiff(item) {
  ...
  if (item.sign === 0) return '-';
  else {
    var diff = [item.diff];
    if (item.sign === -1) diff.push(<span className="down">&darr;</span>);
    else diff.push(<span className="up">&uarr;</span>);
    return diff;
  }
}
  ...
  render() {
    return (
      <Layout.CenteredFullWidth>
        <Header />
        <div className="chart-content">{this.props.children}</div>
      </Layout.CenteredFullWidth>
    );
  }
}
...
function getChartTitle(chart, range) {
  var title = '';
  if (range === 'weekly') title = '주간 ';
  else if (range === 'monthly') title = '월간 ';
  title += chart.title;
  return title;
}
...
class Chart extends React.Component {
  render() {
    const { chart, range } = this.props.data;
    return (
      <ChartLayout>
          ...
          <tbody>
            {chart.items.map(item => (
              <tr>
                ...
                {chart.has_diff && <td className="diff">{renderDiff(item)}</td>}
                <td className="name">
...
export default {
  ...
  async load({ params, loader }) {
    ...
    const [currentUser, chart] = await Promise.all([
      ...
      loader.call(`/charts/${CHART_TYPES[type]}/${range}`, {
        limit: 100,
      }),
    ]);
    return {
